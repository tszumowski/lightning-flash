...
Once we've downloaded the data using :func:`~flash.core.data.download_data`, we create the :class:`~flash.pointcloud.segmentation.data.PointCloudSegmentationData`.
We select a pre-trained ``randlanet_semantic_kitti`` backbone for our :class:`~flash.pointcloud.segmentation.model.PointCloudSegmentation` task.
We then use the trained :class:`~flash.pointcloud.segmentation.model.PointCloudSegmentation` for inference.
